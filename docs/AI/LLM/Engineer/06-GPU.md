...
![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121100854.webp)

显卡由 **显卡核心（GPU） 、电路板（PCB）、显存、金手指、供电 & 显示接口以及散热**等构成。

![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121100941.webp)

显卡核心里主要由运算单元 cuda core，控制单元，缓存单元等构成。

![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121101011.webp)
显存，类似于系统的内存，但它是显卡专用的内存。显存主要用来缓存 GPU 处理过的或者即将提取的渲染数据。显存主要包括容量、频率和位宽这三个参数。容量就是显存的大小，一般来说，显存越大能存储的数据越多，对于部分场景很有用

![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121101048.webp)

输出部分就是显卡挡板处的接口，有：VGA、DVI、HDMI、DP、USB-C（包括雷电 3）

显卡和显示器之间的接口，共有 VGA、HDMI、DVI、DP 以及 USB-C（包括雷电 3）等。

![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121101127.webp)

![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121101207.webp)
...
![image](https://img.philfan.cn/AI__LLM__Engineer__assets__06-GPU.assets__20241121101331.webp)
