...
0x 表示法
0x 表示法是 C 语言中用于表示十六进制数字的标准表示方法。它的形式是一个前缀 0x，后跟一个十六进制数字序列。这个前缀告诉编译器，后面的数字应该被解释为十六进制。
...
小提示：在路径书写时，尽量不要包含空格（用`_`代替），否则容易出错（linux 中空格后内容被自动丢弃）。
