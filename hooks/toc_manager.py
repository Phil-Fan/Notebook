...
class GitManager:
    ...
    def get_latest_commit_time(
        path: Union[str, Path], ignore_commits: Optional[List[str]] = None
    ) -> int:
        """
        获取文件最后一次有效提交的时间戳
...
class TOCManager:
    ...
    def get_file_statistics(
        self, path: Path, ignore_commits: Optional[List[str]] = None
    ) -> Statistics:
        """
        获取文件或目录的统计信息
        ...
        if isinstance(item[title], str):
            ...
            for entry in item[title]:
                ...
                if isinstance(entry, str):
                    path = entry.strip()
                    key = Path(path).stem if path else path
                    value = path
                ...
                # 嵌套分组（value 仍是 list）时递归处理，展平到 contents
                if isinstance(value, list):
                    nested = self.process_toc_item({key: value}, base_path, use_directory_urls)
                    contents.extend(nested.get("contents") or [])
                    continue
    ...
    def process_markdown(self, content: str, page: Page, config: MkDocsConfig) -> str:
        ...
        try:
            ...
            if not isinstance(toc_config, list):
                logger.error("处理目录失败：TOC YAML 必须是列表")
                return content

            for i, item in enumerate(toc_config):
                # 兼容 "- path/to.md" 纯字符串写法，规范为 {标题：路径}
                if isinstance(item, str):
                    path = item.strip()
                    title = Path(path).stem if path else path
                    item = {title: path}
                if not isinstance(item, dict):
                    logger.warning(f"跳过无法解析的 TOC 项：{item!r}")
                    continue
